...
export abstract class InterfaceBuilder {
  ...
  constructor(private commonHelpers: CommonHelpers, private config: PluginConfig) {
    this.commonHelpers.getPrettierOptions().then((prettierOptions) => {
      this.prettierOptions = prettierOptions;
    }).catch((error) => {
      this.commonHelpers.logger.error(`Error while getting prettier options:`, error);
    });
  }
  ...
  public async buildInterfacesFileContent(schema: SchemaInfo) {
    ...
    if (this.prettierOptions) {
      interfacesFileContent = await prettier.format(interfacesFileContent, this.prettierOptions);
    }
