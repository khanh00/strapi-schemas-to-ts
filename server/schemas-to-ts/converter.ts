...
export class Converter {
  ...
  constructor(config: PluginConfig, strapiVersion: string, private readonly strapiDirectories: Core.StrapiDirectories) {
    this.config = config;
  ...
  public async SchemasToTs(): Promise<void> {
    ...
    for (const schema of schemas) {
      const filePath = await this.writeInterfacesFile(schema);
    ...
    if (this.config.generateIndexFile) {
      FileHelpers.generateIndexFilesRecursively(this.destinationPaths, this.commonHelpers.logger);
    }
  }
  ...
  */
  ...
  private async writeInterfacesFile(schema: SchemaInfo): Promise<string> {
    const interfacesFileContent = await this.interfaceBuilder.buildInterfacesFileContent(schema);
